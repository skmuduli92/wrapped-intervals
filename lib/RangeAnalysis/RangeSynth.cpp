...
// well-formed.
void CheckIntervalIsWellFormed(RangeSynth *I, std::string CallerName){
...
// to 1000...0)
bool RangeSynth::isCrossingNorthPole(RangeSynth * S) {
  ...
  RangeSynth NP(NP_lb,NP_ub,width,isSigned);
...
std::vector<RangeSynthPtr> 
ssplit_synth(const APInt &x, const APInt &y, unsigned width, bool IsSigned){

  // Temporary wrapped interval for south pole
  APInt SP_lb = APInt::getMaxValue(width); // 111...1
  APInt SP_ub(width, 0,IsSigned);          // 000...0 
  RangeSynth SP(SP_lb,SP_ub,width,IsSigned);
  // Temporary wrapped interval 
  RangeSynthPtr s(new RangeSynth(x,y,width,IsSigned));

  std::vector<RangeSynthPtr> res;
  if (SP.lessOrEqual(s.get())){
    // Split into two intervals
    RangeSynthPtr s1(new RangeSynth(x,SP_lb,width,IsSigned)); // [x, 111....1]
    RangeSynthPtr s2(new RangeSynth(SP_ub,y,width,IsSigned)); // [000...0,  y] 
    res.push_back(s1);
    res.push_back(s2);
  }
  else{  
...
RangeSynth convertTopToNumInterval(RangeSynth *Op, bool IsSigned){
  if (Op->IsTop()){
    RangeSynth Tmp(*Op);
...
bool RangeSynth::lessOrEqual(AbstractValue* V){
  // Simple cases first: bottom and top
  if (isBot())  return true;  
  RangeSynth *I =  cast<RangeSynth>(V);    
  if (IsTop() && I->IsTop()) 
    return true;
  else{
    if (IsTop())
...
void RangeSynth::join(AbstractValue * V){
  RangeSynth *I2 = cast<RangeSynth>(V);
  RangeSynth *I1 = this;      
...
void RangeSynth::meet(AbstractValue *V1, AbstractValue *V2){

  assert(!isConstant() 
	 &&  "meet can be only called by a non-constant value");

  RangeSynth *I1 = cast<RangeSynth>(V1);
  RangeSynth *I2 = cast<RangeSynth>(V2);
...
bool RangeSynth::isIdentical(AbstractValue *V){
  return RangeSynth::isIdentical(V);
}

bool RangeSynth::isEqual(AbstractValue *V){
  RangeSynth *S = this;
  RangeSynth *T = cast<RangeSynth>(V);    
...
void wideningCousot77(RangeSynth *PreviousI, RangeSynth *CurrentI){
...
#if 0
void widenOneInterval(RangeSynth * Rng, const std::vector<int64_t> &JumpSet, 
		      APInt &lb, APInt &ub){
  ...
  for (unsigned int i=0 ; i<JumpSet.size() ; i++){
	...
	lb = RangeSynth::smax(lb,LandMark) ; 
      if (Rng->getUB().slt(LandMark)) 
	ub = RangeSynth::smin(ub,LandMark) ;
    }
    else{
      if (Rng->getLB().ugt(LandMark)) 
	lb = RangeSynth::umax(lb,LandMark);
      if (Rng->getUB().ult(LandMark)) 
	ub = RangeSynth::umin(ub,LandMark);
    }
  }
...
void wideningJump(RangeSynth *Old, RangeSynth * New, 
...
void RangeSynth::widening(AbstractValue *PreviousV, 
		     const std::vector<int64_t> &JumpSet){
  ...
  case COUSOT76:
    ...
    wideningCousot77(cast<RangeSynth>(PreviousV),this);
    DEBUG(dbgs() << "\tWIDENING (Cousot77) has been applied:" << *this << "\n" );
    return;
  case JUMPSET:
    {
      APInt widenLB, widenUB;
      wideningJump(cast<RangeSynth>(PreviousV), this, JumpSet, widenLB, widenUB);
...
bool RangeSynth::comparisonSle(AbstractValue *V){
...
bool RangeSynth::comparisonSlt(AbstractValue *V){				 
  // if false then it's definite false if true then we still need to
  // check the negation of the condition. If also true, then
  // top. Otherwise, definite true.
...
bool RangeSynth::comparisonUnsignedLessThan(RangeSynth *I1, RangeSynth *I2, bool IsStrict){

  // We need to cut at the south pole. Otherwise, queries like
  // [MININT, 3] <=_u [3,3] returns false, which is incorrect!

  std::vector<RangeSynthPtr> s1 = 
    ssplit_synth(I1->getLB(), I1->getUB(), I1->getLB().getBitWidth(), true);
  std::vector<RangeSynthPtr> s2 = 
    ssplit_synth(I2->getLB(), I2->getUB(), I2->getLB().getBitWidth(), true);
	   
  bool tmp=false;
  typedef std::vector<RangeSynthPtr>::iterator It;
...
bool RangeSynth::comparisonUle(AbstractValue *V){
  RangeSynth * I1 = this;
  RangeSynth * I2 = cast<RangeSynth>(V);
  return comparisonUnsignedLessThan(I1,I2, false);
}

bool RangeSynth::comparisonUlt(AbstractValue *V){				 
  RangeSynth * I1 = this;
  RangeSynth * I2 = cast<RangeSynth>(V);
  return comparisonUnsignedLessThan(I1,I2, false);
}
...
/// false.
void RangeSynth::filterSigma(unsigned Pred,  AbstractValue *V1, AbstractValue *V2){
  RangeSynth *Var1   = cast<RangeSynth>(V1);
  RangeSynth *Var2   = cast<RangeSynth>(V2);
...
void RangeSynth::filterSigma_VarAndConst(unsigned Pred, RangeSynth *V, RangeSynth *N){
...
void RangeSynth::filterSigma_TwoVars(unsigned Pred, RangeSynth *I1, RangeSynth *I2){
  ...
  case ICmpInst::ICMP_SLE: // I1 <= I2
    {
      ...
      RangeSynth tmp1 = convertTopToNumInterval(I1,IsSignedOp);
      APInt a = tmp1.getLB();
      APInt b = tmp1.getUB();
      APInt c = I2->getLB();
      APInt d = I2->getUB();
    ...
    return;
    // if (I1->IsTop() || RangeSynth::bridge_IsIncluded(Pred,
    // 				     I2->getLB(),I2->getUB(),
    // 				     I1->getLB(),I1->getUB())){
    ...
    //   return;
    // }
    // if (RangeSynth::bridge_IsOverlapLeft(Pred,
    // 					I1->getLB(),I1->getUB(),
    // 					I2->getLB(),I2->getUB())){
  ...
  case ICmpInst::ICMP_SGE: 
    ...
    return;
    // if (RangeSynth::bridge_IsIncluded(Pred,
    // 				     I2->getLB(),I2->getUB(),
    // 				     I1->getLB(),I1->getUB())){
    ...
    //   return;
    // }
    // if (RangeSynth::bridge_IsOverlapRight(Pred,
    // 					 I1->getLB(),I1->getUB(),
    // 					 I2->getLB(),I2->getUB())){
    //   // The result is the meet between I1 and I2
    //   return;
...
AbstractValue* RangeSynth::
visitArithBinaryOp(AbstractValue* V1, AbstractValue* V2,
		   unsigned OpCode, const char * OpCodeName ){
  
  RangeSynth *Op1 = cast<RangeSynth>(V1);
  RangeSynth *Op2 = cast<RangeSynth>(V2);
  RangeSynth *LHS = new RangeSynth(*this);
...
void RangeSynth::DoArithBinaryOp(RangeSynth *LHS, RangeSynth *Op1, RangeSynth *Op2,
			    unsigned OpCode, const char *OpCodeName, 
			    bool &IsOverflow){
  // For debugging:
...
void RangeSynth:: DoMultiplication(bool IsSignedOp,
			      RangeSynth *LHS, RangeSynth *Op1, RangeSynth *Op2,
			      bool &IsOverflow){
  APInt a,b,c,d;
...
/// else                    { [a,b] }
std::vector<RangeSynthPtr> 
purgeZeroSynth(const APInt &x, const APInt &y, unsigned width, bool IsSignedOp){
  ...
  RangeSynth Zero(Zero_lb,Zero_ub,width,IsSigned);
  // Temporary wrapped interval 
  RangeSynthPtr s(new RangeSynth(x, y, width, IsSigned));
  std::vector<RangeSynthPtr> res;
  if (Zero.lessOrEqual(s.get())){
    if (x == 0 ){
      ...
      RangeSynthPtr s(new RangeSynth(x+1,y,width,IsSigned)); 
      res.push_back(s);      
    }
    else{
      if (y == 0){
	...
	RangeSynthPtr s(new RangeSynth(x,minusOne,width,IsSigned)); // [x, 111....1]
	res.push_back(s);	
      }
      else{
	...
	APInt minusOne = APInt::getMaxValue(width); // 111...1
	RangeSynthPtr s1(new RangeSynth(x,minusOne,width,IsSigned)); // [x, 111....1]
	RangeSynthPtr s2(new RangeSynth(plusOne,y,width,IsSigned)); // [000...1,  y] 
	res.push_back(s1);
	res.push_back(s2);
      }
    }
...
void RangeSynth::DoDivision(bool IsSignedOp,
		       RangeSynth *LHS, RangeSynth *Dividend, RangeSynth *Divisor,
		       bool &IsOverflow){
  ...
  std::vector<RangeSynthPtr> s1 = 
    purgeZeroSynth(Divisor->getLB(), Divisor->getUB(), 
  ...
  for (std::vector<RangeSynthPtr>::iterator Divisors = s1.begin(), 
...
///
void DoRemCasesSynth(const APInt &a, const APInt &b, const APInt &c, const APInt &d, 
...
void RangeSynth::DoRem(bool IsSignedOp,
		  RangeSynth *LHS, RangeSynth *Dividend, RangeSynth *Divisor,
		  bool &IsOverflow){
  ...
  std::vector<APInt> extremes;
  for (std::vector<RangeSynthPtr>::iterator Divisors = s1.begin(), E1 =s1.end(); 
       Divisors != E1; ++Divisors){
    APInt c = (*Divisors).get()->getLB();
    APInt d = (*Divisors).get()->getUB();
    APInt lb, ub;
    DoRemCasesSynth(a,b,c,d,width,IsSignedOp,lb,ub);
...
visitCast(Instruction &I,  AbstractValue * V, TBool * TB, bool IsSigned){
  ...
  if (!V){
    ...
    RHS = new RangeSynth(I.getOperand(0), TB, IsSigned); 
  }
  else{
    // Common case
    RHS = cast<RangeSynth>(V);    
...
bool RangeSynth::IsTruncateOverflow(RangeSynth * RHS, unsigned destWidth){
  ...
  // not fit into 3 bits. (remember RangeSynth assumes that
...
/// overflow.
void RangeSynth::
DoCast(RangeSynth *LHS, RangeSynth *RHS, 
...
void RangeSynth:: 
  DoBitwiseBinaryOp(RangeSynth * LHS, RangeSynth * Op1, RangeSynth * Op2, 
		    const Type * Op1Ty, const Type * Op2Ty, 
...
void RangeSynth::
  DoBitwiseShifts(RangeSynth *LHS, RangeSynth *Operand, RangeSynth * Shift,
		  unsigned OpCode, bool &IsOverflow){
...
void RangeSynth::DoLogicalBitwise(RangeSynth *LHS, RangeSynth *Op1, RangeSynth *Op2,
			     unsigned Opcode){
  if (Op1->isBot() || Op2->isBot()){
  ...
  case Instruction::Or:    
    {
      RangeSynth Tmp1 = convertTopToNumInterval(Op1, isSigned);
      RangeSynth Tmp2 = convertTopToNumInterval(Op2, isSigned);
...
// 000...0)
bool RangeSynth::isCrossingSouthPole(RangeSynth * S){
  unsigned width    = S->getWidth();
  unsigned isSigned = S->IsSigned();
  APInt SP_lb = APInt::getMaxValue(width); // 111...1
  APInt SP_ub(width, 0,isSigned);          // 000...0 
...
/// book.
void RangeSynth::signedOr(RangeSynth *Op1, RangeSynth*Op2){
...
void RangeSynth::signedAnd(RangeSynth *Op1, RangeSynth *Op2){
  bool IsSigned = true;
  // Cut at the south poles, apply unsigned AND on each pair of
  // intervals, and join them.
  std::vector<RangeSynthPtr> s1 = 
    ssplit_synth(Op1->getLB(), Op1->getUB(), Op1->getLB().getBitWidth(), IsSigned);
  std::vector<RangeSynthPtr> s2 = 
    ssplit_synth(Op2->getLB(), Op2->getUB(), Op2->getLB().getBitWidth(), IsSigned);

  this->makeBot();  
  for (std::vector<RangeSynthPtr>::iterator I1 = s1.begin(), E1 =s1.end(); I1 != E1; ++I1){
    for (std::vector<RangeSynthPtr>::iterator I2 = s2.begin(), E2 =s2.end(); I2 != E2; ++I2){
      APInt lb; APInt ub;
      unimelb::unsignedAnd((*I1).get(), (*I2).get(), lb, ub);
      RangeSynth tmp(lb,ub, Op1->getWidth(), Op1->IsSigned());  
      this->join(&tmp);
    }
  }
}

void RangeSynth::signedXor(RangeSynth *Op1, RangeSynth *Op2){
  bool IsSigned = true;
  // Cut at the south poles, apply unsigned AND on each pair of
  // intervals, and join them.
  std::vector<RangeSynthPtr> s1 = 
